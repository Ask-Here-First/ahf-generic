...
from .finder import scan_transforms, str_find_any
...
class TestFinder(unittest.TestCase):
    ...
    def test_scan_transforms(self):
        s = "a3b4c5"
        self.assertEqual(scan_transforms(s, {'a': self._add_next_by_one}),
                         (len(s), "a4b4c5"))
        self.assertEqual(scan_transforms(s, {'b': self._add_next_by_one}),
                         (len(s), "a3b5c5"))
        self.assertEqual(scan_transforms(s, {'a': self._add_next_by_one}),
                         (len(s), "a4b4c5"))
        self.assertEqual(scan_transforms(s, {'d': self._add_next_by_one}),
                         (len(s), "a3b4c5"))
        self.assertEqual(scan_transforms(s, {
            'a': self._add_next_by_one, 'c': self._add_next_by_one, 'd': self._add_next_by_one
        }), (len(s), "a4b4c6"))
        self.assertEqual(scan_transforms(
